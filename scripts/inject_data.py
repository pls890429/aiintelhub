...
def build_action(vendor, title, product):
    if product == '价格动作':
        ...
        implication = f'{vendor}在重点行业或标杆客户上强化攻势，华为云需尽快识别同类客户场景，准备盘古大模型、昇腾算力及行业解决方案的替代话术。'
        script = '友商已经在类似行业释放标杆案例，建议我们尽快交流贵司 AI 场景规划，看看华为云是否能提供更稳、更安全、更贴近行业的方案。'
    elif product == '产品/模型更新':
        implication = f'{vendor}产品/模型能力持续更新，销售侧应关注客户是否因此重新评估云厂商能力，并及时同步华为云盘古、昇腾、ModelArts 等方案进展。'
